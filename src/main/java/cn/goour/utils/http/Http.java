...
public class Http {
	public static void main(String[] args) {
		System.out.println();
		File f = new File("D:\\workspace\\eclipseJeeNeonWorkspace\\yiban-app\\target\\classes\\pic");
...
//		System.out.println(Arrays.toString(files));
		...
		config = new HttpConfig("http://goour.cn/server.php?sd=dsdsd");
...
//			System.out.println(aa);
	...
	/**
	 * 不建议使用该方法，如需上传文件，请使用post(HttpConfig config, Map<String, Object> mapObj, Map<String, File[]> mapFiles)方法，
	 * 把第二参数置为null
	 * @param config
	 * @param files
	 * @param key
	 ...
	 * HTTP POST请求，文件上传请求
	 * @param config
	 * @param mapObj 表单信息，可为null
	 * @param mapFiles 文件信息，可为null
	 * @return
	 * @throws Exception
	 */
	...
	public static void showHeaders(Map<String, List<String>> map) {
		...
		// System.out.println("赋值后："+yVo.getCookieString());
...
//		System.out.println("HTTP Url  :"+url.getProtocol()+"://"+url.getHost()+":"+(url.getPort()==-1?url.getDefaultPort():url.getPort())+url.getPath());
...
//		showHeaders(conn.getHeaderFields());
		...
		while (conn.getHeaderField("Location") != null) {
			...
			locationUrl = getAbsUrl(config.getUrl(), locationUrl);//.replaceAll("^https", "http");
...
//			showHeaders(conn.getHeaderFields());
	...
	/**
	 * 以HTTPS协议初始化请求，但是这个没办法用，一直提示证书错误，参照了网上的很多方法都没有解决，心累
	 * @param url
	 * @return
	 * @throws IOException
	 * @throws Exception
